...
def runPMH(pars, builders):
	...
	contentMaker, getArgs = builders[verb]

	# dispatch on prefix if necessary (contentMaker is a dict)
	if isinstance(contentMaker, dict):
		contentMaker = dispatchOnPrefix(pars, contentMaker)
...
def makeResumptionToken(pars, nextOffset):
	...
	return utils.getCleanBytes(
			urlparse.urlencode(toEncode)).decode("ascii")
...
def parseResumptionToken(pars):
	...
	"""
	try:
		newPars = dict(urllparse.parse_qsl(
			utils.getDirtyBytes(pars["resumptionToken"]).decode("ascii")))
...
def dispatchOnPrefix(pars, contentMakers):
	"""returns a resource factory depending on the metadataPrefix in pars.

	contentMakers is one of the dictionaries called "verb" in builders
	below.

	Invalid metadataPrefixes are detected here and lead to exceptions.
	"""
	try:
		return contentMakers[pars["metadataPrefix"]]
	except KeyError:
...
def _addTemporalCondition(parVal, operator, sqlFrags, sqlPars, parName):
	...
	if not parsed and mat:
		...
		# postgres compares dates against timestamps as if the dates were for
...
def getMatchingRows(pars, rscTableDef, getSetFilter):
	...
	try:
		...
		if len(res)==maxRecords:
			# there's probably more data, add a resumption token
...
class RegistryCore(svcs.Core, base.RestrictionMixin):
	...
	# builders maps actions to a pair of (thing, function(pars) -> whatever),
	# where function accepts the oai-pmh parameters and returns the an internal
	# representation of the response.  thing is either a function accepting
	# this internal representation for verbs not requiring a metadata
	# prefix, or it is a mapping of metadata prefixes to such functions
	# otherwise.
	builders = {
		"GetRecord": ({
				"ivo_vor": builders.getVOGetRecordElement,
				"oai_dc": builders.getDCGetRecordElement},
			lambda pars: (identifiers.getResobFromIdentifier(pars["identifier"]),)),
		"ListRecords": ({
				"ivo_vor": builders.getVOListRecordsElement,
				"oai_dc": builders.getDCListRecordsElement},
			lambda pars: (getMatchingResobs(pars), _getSetNames(pars))),
		"ListIdentifiers": ({
				"ivo_vor": builders.getListIdentifiersElement,
				"oai_dc": builders.getListIdentifiersElement},
			lambda pars: (getMatchingRestups(pars),)),
		"ListSets": (builders.getListSetsElement,
				lambda pars: ()),
	...
	def run(self, service, inputTable, queryMeta):
		...
		described by pars.
		"""
		args = inputTable.getParam("args") or {}
